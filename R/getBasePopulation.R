...
getBasePopulation <- function(nReps){
...
#### (80-20) data split for training and test data set for training GS models   
    ...
    initseed <- 25+nrep 
	set.seed(initseed)
    trainIndices <-sample(c(1:nLines),(0.8*nLines)) 
...
#### Training Set table for Genotype and Simulated Phenotype ################### 
    ...
    trainGenoNewTable <- genoTable_New[trainIndices,] 
	trainGenoNewTable_w_o_QTL <- genoTable_New[trainIndices,-c(QTL_Indices)] 
    
	trainSimPhenoValuesTable <- phenotypicValuesSimTable[trainIndices,] 
    trainSimGenoValuesTable <-  genotypicValuesSimTable[trainIndices,]
      
#### Validation Set table for Genotype and Simulated Phenotype ###################
      
    testGenoTable<- genoTable_Mod[testIndices,] 
    testGenoNewTable <- genoTable_New[testIndices,]
	testGenoNewTable_w_o_QTL <- genoTable_New[testIndices,-c(QTL_Indices)]
	
    testSimPhenoValuesTable <- phenotypicValuesSimTable[testIndices,]  
    testSimGenoValuesTable <-  genotypicValuesSimTable[testIndices,]
 
#### List of Tables for nCondition (combination of QTL & h2 condition) #######################################     
    
	phenotypicValuesSimTableList[[nCondition]] <- phenotypicValuesSimTable
    genotypicValuesSimTableList[[nCondition]] <- genotypicValuesSimTable
	
    phenotypicValuesSimList[[nCondition]] <- phenoSimValues
    genotypicValuesSimList[[nCondition]] <- genoSimValues
    errorVarList[[nCondition]] <- errorVar
      
    trainIndicesList[[nCondition]] <- trainIndices 
    testIndicesList[[nCondition]] <- testIndices 
      
      
    trainGenoTableList[[nCondition]] <- trainGenoTable
    trainGenoNewTableList[[nCondition]] <- trainGenoNewTable
	trainGenoNewTableList_w_o_QTL[[nCondition]] <- trainGenoNewTable_w_o_QTL
    trainSimPhenoValuesTableList[[nCondition]] <- trainSimPhenoValuesTable
    trainSimGenoValuesTableList[[nCondition]] <- trainSimGenoValuesTable
      
    testGenoTableList[[nCondition]] <- testGenoTable
    testGenoNewTableList[[nCondition]] <- testGenoNewTable
	testGenoNewTableList_w_o_QTL[[nCondition]] <- testGenoNewTable_w_o_QTL
	
    testSimPhenoValuesTableList[[nCondition]] <- testSimPhenoValuesTable
    testSimGenoValuesTableList[[nCondition]] <- testSimGenoValuesTable
